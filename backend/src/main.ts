...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .addTag(
      'Auth',
      "Endpoints d'authentification : inscription, connexion, réinitialisation du mot de passe",
    )
    .addTag('Profile', 'Gestion du profil utilisateur connecté')
    .addTag(
      'Quality - Contrôle Qualité',
      'Gestion des dates de contrôle, lignes et rapports qualité',
    )
    .addTag(
      'Super Admin',
      'Gestion des utilisateurs, approbation, logs et statistiques',
    )
    .addBearerAuth(
      {
        type: 'http',
