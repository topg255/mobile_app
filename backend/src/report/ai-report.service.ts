...
export class AiReportService {
  ...
  async collectKPIs(targetDate: Date): Promise<ReportKPIs> {
    ...
    if (lignes.length === 0) {
      ...
      lignes = await this.ligneRepo
        ...
        .where('ligne.created_at BETWEEN :start AND :end', { start: startOfDay, end: endOfDay })
        .getMany();
    }
    ...
    const vertPercent = totalLignes > 0 ? Math.round((vertCount / totalLignes) * 100 * 100) / 100 : 0;
    const jaunePercent = totalLignes > 0 ? Math.round((jauneCount / totalLignes) * 100 * 100) / 100 : 0;
    const rougePercent = totalLignes > 0 ? Math.round((rougeCount / totalLignes) * 100 * 100) / 100 : 0;
    ...
    lignes.forEach((l) => {
      if (l.agent) {
        ...
        if (existing) {
          ...
          agentMap.set(key, { name: `${l.agent.firstName} ${l.agent.lastName}`, count: 1 });
        }
      }
    });
    let topAgent = 'Aucun';
  ...
  async generateReport(targetDate: Date, superviseurName: string): Promise<GeneratedReport> {
